...
def _enforce_non_streaming(stream_value: Optional[Any], partial_images_value: Optional[Any]):
    if _coerce_bool(stream_value):
        raise HTTPException(status_code=501, detail="Streaming responses are not supported by this service yet.")
    if _has_positive_value(partial_images_value):
        raise HTTPException(status_code=501, detail="partial_images requires streaming, which is not supported by this service yet.")
...
async def openai_generations(request: Request):
    ...
    unified_req = RequestMapper.openai_gen_to_unified(body, config, provider=config["provider"])
...
async def openai_edits(
    ...
    logger.info(f"Params (OpenAI Edit): model={model}, prompt={prompt}, size={log_size}, "
...
async def gemini_generate_content(model_name: str, request: Request):
    ...
    # Map to Unified
    unified_req = RequestMapper.gemini_content_to_unified(body, model_name, config, provider=config["provider"])
